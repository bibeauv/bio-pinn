...
class Discovery():
    ...
    def loss(self, x, y_train):
        ...
        self.loss_cTG_ode = self.loss_function_ode(grad_cTG + k*cTG, self.f_hat)
        self.loss_T_ode = self.loss_function_ode(grad_T * self.prm.rho * self.prm.Cp * self.prm.V - \
                                                      self.prm.epsilon*Q - \
                                                      self.prm.dHrx * self.prm.V/1000 * grad_cTG, self.f_hat)
        ...
        self.total_loss = self.loss_cTG_ode + self.loss_T_ode + self.loss_T_data + 1000*self.loss_cTG_data + self.loss_k
