...
PINN = Discovery(X_train, Y_external, y0, idx, idx_y0, f_hat, device, prm,
                 Ea=320.0, A=6.9)
...
while epoch <= epochs:
    ...
    if epoch % 100 == 0:
        print(f'Epoch {epoch}, \t cTG_data_loss: {PINN.loss_cTG_data:.4e} \t T_ode_loss: {PINN.loss_T_ode:.4e} \t cTG_ode_loss: {PINN.loss_cTG_ode:.4e} \t k_loss: {PINN.loss_k:.4e} \t Ea: {PINN.Ea:.2f} \t A: {PINN.A:.2f}')
    if epoch == 1000:
