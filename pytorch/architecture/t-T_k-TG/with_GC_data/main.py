...
while epoch <= epochs:
    ...
    if epoch % 100 == 0:
        print(f'Epoch {epoch}, loss: {PINN.total_loss:.2e} cTG_data: {PINN.loss_cTG_data:.2e} T_ode: {PINN.loss_T_ode:.2e} cTG_ode: {PINN.loss_cTG_ode:.2e}')
    if epoch == 700:
...
plt.plot(X_train[idx1_,0].detach().numpy(), PINN.PINN(X_train)[idx1_,0].detach().numpy(), label='2 min')
plt.plot(X_train[idx_y0,0].detach().numpy(), y0, 'o', label='Conditions')
plt.plot(X_train[idx2_,0].detach().numpy(), PINN.PINN(X_train)[idx2_,0].detach().numpy(), label='4 min')
plt.plot(X_train[idx3_,0].detach().numpy(), PINN.PINN(X_train)[idx3_,0].detach().numpy(), label='8 min')
plt.legend()
plt.xlabel('Time [sec]')
plt.ylabel(r'Concentration [mol/L]')
plt.show()

plt.plot(X_train[idx1_,1].detach().numpy(), PINN.PINN(X_train)[idx1_,1].detach().numpy(), 'o', label='2 min')
plt.plot(X_train[idx2_,1].detach().numpy(), PINN.PINN(X_train)[idx2_,1].detach().numpy(), 'o', label='4 min')
plt.plot(X_train[idx3_,1].detach().numpy(), PINN.PINN(X_train)[idx3_,1].detach().numpy(), 'o', label='8 min')
plt.legend()
plt.xlabel(r'Temperature [$\degree$C]')
plt.ylabel(r'Kinetic constant [s$^{-1}$]')
plt.show()

plt.plot(X_train[idx1_,0].detach().numpy(), PINN.PINN(X_train)[idx1_,1].detach().numpy(), label='2 min')
plt.plot(X_train[idx2_,0].detach().numpy(), PINN.PINN(X_train)[idx2_,1].detach().numpy(), label='4 min')
plt.plot(X_train[idx3_,0].detach().numpy(), PINN.PINN(X_train)[idx3_,1].detach().numpy(), label='8 min')
plt.legend()
plt.xlabel('Time [sec]')
plt.ylabel(r'Kinetic constant [s$^{-1}$]')
plt.show()