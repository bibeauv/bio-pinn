...
class Curiosity():
    ...
    def loss(self, x, y_train):
        ...
        grad_T = torch.gradient(T.detach().flatten(), spacing=(t.detach().flatten(),))[0].reshape(-1,1)
...
class Discovery():
    ...
    def __init__(self, X, Y_external, y0, idx, idx_y0, f_hat, device, prm, Ea, A):
    ...
    def loss(self, x, y_train):
        ...
        self.total_loss = self.loss_cTG_ode + self.loss_T_ode + 1e6*self.loss_cTG_data + self.loss_k
