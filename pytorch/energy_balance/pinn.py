...
T_train, \
cTG_train, \
y_pred, idx = read_data(Ea, A, Q, t1, t2, collocation_points, device, prm)
...
# Inputs
x_train_cTG = t_train
x_train_Temp = torch.cat((t_train, Q_train),1)
...
class parametres():
    ...
    dHrx = -50
prm_mw = parametres()
PINN_cTG = cTGNN(device, f_hat, y_pred, k_pred, x_train_cTG, cTG_train, idx)
PINN_Temp = TempNN(device, f_hat, y_pred, k_pred, x_train_Temp, T_train, prm_mw, idx)
...
# Initial predictions
y_pred['cTG'] = PINN_cTG.dnn(x_train_cTG).detach().clone().flatten()
y_pred['Temp'] = PINN_Temp.dnn(x_train_Temp).detach().clone().flatten()
...
while epoch < max_epochs:
    # Backward
    train_cNN(PINN_cTG, 'cTG', y_pred, k_pred, x_train_cTG)
    vec_loss_cTG.append(float(PINN_cTG.loss(x_train_cTG, cTG_train).detach().cpu().numpy()))
    
    train_cNN(PINN_Temp, 'Temp', y_pred, k_pred, x_train_Temp)
    vec_loss_Temp.append(float(PINN_Temp.loss(x_train_Temp, T_train).detach().cpu().numpy()))

    epoch += 1
    
    if epoch % 100 == 0:
        print(f'Epoch {epoch}, \t cTG_loss: {vec_loss_cTG[-1]:.4e} \t Temp_loss: {vec_loss_Temp[-1]:.4e} \t Ea: {PINN_Temp.k_pred[0]:.1f} \t A: {PINN_Temp.k_pred[1]:.1f}')
...
axs[0].plot(t_train[2002:].detach().cpu().numpy(), PINN_cTG.dnn(x_train_cTG)[2002:].detach().cpu().numpy(), label='PINN')
axs[0].plot(t_num, y_num[:,0], '--', label='Euler')
axs[0].plot(t_num2, y_num2[:,0], label='Real')
axs[0].set_ylabel('Concentration [mol/L]')
axs[0].legend()
axs[1].plot(t_train[2002:].detach().cpu().numpy(), PINN_Temp.dnn(x_train_Temp)[2002:].detach().cpu().numpy(), label='PINN')
