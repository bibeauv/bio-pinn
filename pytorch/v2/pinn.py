...
class subDNN(nn.Module):
    ...
    def __init__(self, neurons, activation, params, idx, device):
        ...
        self.k1 = torch.tensor([params[0]], requires_grad=True).float().to(device)
        self.k2 = torch.tensor([params[1]], requires_grad=True).float().to(device)
        self.k3 = torch.tensor([params[2]], requires_grad=True).float().to(device)
        self.k4 = torch.tensor([params[3]], requires_grad=True).float().to(device)
        self.k5 = torch.tensor([params[4]], requires_grad=True).float().to(device)
        self.k6 = torch.tensor([params[5]], requires_grad=True).float().to(device)
...
class PINN():
    ...
    def loss(self, x, y):
        ...
        loss_cTG_ode = self.loss_function_ode(grad_cTG + self.nn.k1*self.cTG - self.nn.k2*self.cDG*self.cB, self.f_hat)
        ...
        self.loss_ode = loss_cB_ode + loss_cTG_ode + loss_cDG_ode + loss_cMG_ode + loss_cG_ode
