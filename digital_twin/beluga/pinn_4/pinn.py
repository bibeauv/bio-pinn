...
class Curiosity():
    ...
    def loss(self, x, y_train):
        ...
        k1 = self.PINN.A1 + self.PINN.E1 * (T - torch.min(T))
        k2 = self.PINN.A2 + self.PINN.E2 * (T - torch.min(T))
        k3 = self.PINN.A3 + self.PINN.E3 * (T - torch.min(T))
        k4 = self.PINN.A4 + self.PINN.E4 * (T - torch.min(T))
        k5 = self.PINN.A5 + self.PINN.E5 * (T - torch.min(T))
        k6 = self.PINN.A6 + self.PINN.E6 * (T - torch.min(T))
