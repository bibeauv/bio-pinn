...
def EDO(y, prm, i):
    ...
    k1 = prm.A1 + prm.E1 * (Ti - np.mean(T))
    k2 = prm.A2 + prm.E2 * (Ti - np.mean(T))
    k3 = prm.A3 + prm.E3 * (Ti - np.mean(T))
    k4 = prm.A4 + prm.E4 * (Ti - np.mean(T))
    k5 = prm.A5 + prm.E5 * (Ti - np.mean(T))
    k6 = prm.A6 + prm.E6 * (Ti - np.mean(T))
