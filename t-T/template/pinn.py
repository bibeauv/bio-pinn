...
class Curiosity():
    ...
    def __init__(self, X, Y, idx, f_hat, learning_rate, E, A, e, c1, c2, neurons, regularization, device, prm):
    ...
    def loss(self, x, y_train):
        ...
        self.loss_cME_ode = self.loss_function_ode(grad_cME - k1*cTG + k2*cDG*cME \
                                                            - k3*cDG + k4*cMG*cME \
                                                            - k5*cMG + k6*cG*cME, self.f_hat)
        self.loss_T_ode = self.loss_function_ode(self.prm.m_Cp*grad_T - self.PINN.e*self.prm.Q - self.prm.dHrx1 * grad_cTG * self.prm.V \
                                                                                               - self.prm.dHrx2 * grad_cDG * self.prm.V \
                                                                                               - self.prm.dHrx3 * grad_cMG * self.prm.V \
