...
def gather_data(files):
    ...
    Q = np.linspace(C[0,1], C[-1,1], int(C[-1,0])+1)
    ...
    for i in range(1,len(files)):
        ...
        new_t = np.linspace(new_C[0,0], new_C[-1,0], int(new_C[-1,0])+1)
        new_Q = np.linspace(new_C[0,1], new_C[-1,1], int(new_C[-1,0])+1)
        new_idx = find_idx(new_t, new_C)

        new_X = np.concatenate((new_t.reshape(-1,1), new_Q.reshape(-1,1)), axis=1)
        X = np.concatenate((X, new_X), axis=0)
        Y = np.concatenate((Y, new_C[:,-1].reshape(-1,1)), axis=0)
