...
class Curiosity():
    ...
    def __init__(self, X, Y, idx, f_hat, device, prm):
        ...
        self.E1 = torch.tensor(156., requires_grad=True).float().to(device)
        self.A1 = torch.tensor(1., requires_grad=True).float().to(device)
        self.E2 = torch.tensor(199., requires_grad=True).float().to(device)
    ...
    def loss(self, x, y_train):
        ...
        self.loss_c_ode = self.loss_cA_ode + self.loss_cB_ode
        self.loss_c_data = self.loss_cA_data + self.loss_cB_data
