...
class Curiosity():
    ...
    def __init__(self, X, Y, idx, f_hat, device, prm):
        ...
        self.Ea = torch.tensor(95.2, requires_grad=True).float().to(device)
        self.A = torch.tensor(0.072, requires_grad=True).float().to(device)
    ...
    def loss(self, x, y_train):
        ...
        # self.loss_T_ode = self.loss_function_ode(grad_T - self.prm.e*self.prm.Q - self.prm.dHrx * grad_c * self.prm.V - self.prm.c1*T - self.prm.c2, self.f_hat)
        ...
        self.total_loss = self.loss_c_ode + self.loss_c_data #+ self.loss_T_data #+ self.loss_k
