...
class subnetA():
    ...
    def __init__(self, X, Y, idx, f_hat, device, prm):
        ...
        self.A = torch.tensor(1., requires_grad=True).float().to(device)
    ...
    def loss(self, x, y_train):
        ...
        k1 = self.A * torch.exp(-self.E / self.PINNT.T.detach())
        
        self.grad_c = autograd.grad(self.c, g, torch.ones(x.shape[0], 1).to(self.device), \
                                    retain_graph=True, create_graph=True) \
                                    [0]

        self.loss_c_ode = self.loss_function_ode(self.grad_c + k1*self.c, self.f_hat)
...
class subnetT():
    ...
    def loss(self, x, y_train):
        ...
        grad_T = autograd.grad(self.T, g, torch.ones(x.shape[0], 1).to(self.device), \
                               retain_graph=True, create_graph=True) \
                               [0]

        self.loss_T_ode = self.loss_function_ode(self.prm.m_Cp*grad_T - self.e*self.prm.Q - self.prm.dHrx * self.PINNA.grad_c.detach() * self.prm.V - self.c1*self.T - self.c2, self.f_hat)
        
        self.loss_T_data = self.loss_function_data(self.T, y_train[:,1].reshape(-1,1))
